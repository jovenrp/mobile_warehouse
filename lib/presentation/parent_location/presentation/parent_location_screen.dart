                                          ...
                                          showDialog(
                                              ...
                                              builder: (BuildContext context) {
                                                return StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
                                                  return addChild(
                                                      state.containerModel?[index], isDialogueError, setState);
                                                });
                                              }).then((_) {
        ...
        bottomNavigationBar: state.containerModel?.isNotEmpty == true
            ? (parentName == 'Root' || state.containerModel?[0].isRoot?.toLowerCase() == 'y')
                ...
                : Padding(
                    ...
                    child: ATTextButton(
                        buttonText: widget.navigation == 'push' || parentName?.toLowerCase() == 'root'
                            ? 'Create a child for $parentName'
                            : 'Create a child for $rootName',
                        isLoading: state.isLoading,
                        onTap: () => showDialog(
                                context: context,
                                builder: (BuildContext context) {
                                  return StatefulBuilder(builder: (BuildContext context, StateSetter state) {
                                    return parentContainer?.isNotEmpty == true ? addChild(parentContainer?.first, isDialogueError, state) : addChild(widget.container, isDialogueError, state);
                                  });
                                }).then((_) {
                              isDialogueError = false;
                            })))
            ...
            children: <Widget>[
              ...
              Row(
                children: <Widget>[
                  ...
                  Flexible(
                      child: ATText(
                          text: 'Fill up the field to create a new child for ${containerModel?.name?.isNotEmpty == true ? '${containerModel?.name}' : containerModel?.code}'))
