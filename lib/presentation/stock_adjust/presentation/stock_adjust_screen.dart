                ...
                Padding(
                  ...
                  child: ATSearchfield(
                    ...
                    onFieldSubmitted: (String? value) {
                      ...
                      } else if (locationController.text.trim().isNotEmpty == true && searchController.text.trim().isNotEmpty == true) {
                        context.read<StockAdjustBloc>().stockLookUp(sku: searchController.text, locNum: locationController.text);
                      }
                    },
                    onPressed: () async {
                      String? code = await Navigator.push(context, MaterialPageRoute<String>(builder: (BuildContext context) {
                        return QRStockAdjustScreen(sku: searchController.text.trim().isNotEmpty ? searchController.text : '');
                      }));
                          ...
                          MaterialPageRoute<String>(
                              builder: (BuildContext context) {
                                return QRStockAdjustScreen(
                                    location: locationController.text.trim().isNotEmpty ? locationController.text : '');
                              }));
                        ...
                        ? Container(
                            ...
                            child: state.stockItems?.isNotEmpty == true
                                ? Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.start, children: <Widget>[
                                    Visibility(
                                        visible: state.stockItems?.isNotEmpty == true,
                                        child: Table(
                                    ...
                                    Flexible(
                                      child: ListView.builder(
                                          itemCount: (state.stockItems?.length ?? 0),
                                          shrinkWrap: true,
                                          itemBuilder: (BuildContext context, int index) {
                                            qtyControllers.add(TextEditingController());
                                            adjustControllers.add(TextEditingController());
                                            if (!isInit) {
                                              qtyControllers[index].text = state.stockItems?[index].qty ?? '0';
                                              if (index == state.stockItems!.length - 1) {
                                                isInit = true;
                                              }
                                            }
                                            return Table(
                                                ...
                                                children: <TableRow>[
                                                  TableRow(decoration: BoxDecoration(color: AppColors.beachSeaTint20), children: <Widget>[
                                                    Container(
                                                      padding: const EdgeInsets.only(left: 10, bottom: 6, top: 6),
                                                      child: ATText(
                                                        text: state.stockItems?[index].sku,
                                                        fontSize: 16,
                                                        fontColor: AppColors.white,
                                                      ),
                                                    ),
                                                    Container(
                                                      ...
                                                      child: ATText(
                                                          text: '${state.stockItems?[index].qty} ${state.stockItems?[index].uom}',
                                                          fontSize: 16,
                                                          fontColor: AppColors.white),
                                                    ),
                                                  ]),
                                                  TableRow(decoration: BoxDecoration(color: AppColors.beachSeaTint20), children: <Widget>[
                                                    Container(
                                                      padding: const EdgeInsets.only(left: 10, bottom: 6, top: 6),
                                                      child: ATText(
                                                        text: state.stockItems?[index].name,
                                                        fontSize: 16,
                                                        fontColor: AppColors.white,
                                                      ),
                                                    ),
                                          ...
                                          child: ATTextfield(
                                              ...
                                              onFieldSubmitted: (String? value) {
                                                context
                                                    ...
                                                    .stockAdjust(
                                                        absolute: false,
                                                        containerId: state.stockItems?.first.containerId,
                                                        sku: state.stockItems?.first.sku,
                                                        qty: value)
                                                    .then((List<StockAdjustModel>? stockModel) {
                                                  quantityControllers.clear();
                                                  context
                                                      .read<StockAdjustBloc>()
                                                      .stockLookUp(sku: searchController.text, locNum: locationController.text);
                                                });
                                              })),
                                    )
                                  ])
